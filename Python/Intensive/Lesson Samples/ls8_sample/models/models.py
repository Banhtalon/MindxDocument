...
class AnimeItem:
    ...
    # giải thích phương thức update trong class AnimeItem: 
    # new_data: là một đối tượng dict.
    # .items() là một phương thức trả về một view object hiển thị danh sách các cặp khóa-giá trị của một dictionary.    
    # Ở đây, nó lặp qua từng cặp khóa-giá trị trong đối tượng new_data.
    # Đối với mỗi cặp, nó gán khóa cho biến 'attribute' và giá trị cho biến 'value'.
    # Nếu giá trị mới khác rỗng (chỉ khi user nhập dữ liệu mới) thì mới update
    # setattr(self, attribute, value) là một phương thức built-in của Python dùng để thiết lập giá trị của một thuộc 
    # tính của đối tượng.
    # Trong trường hợp này, nó sẽ thiết lập giá trị của thuộc tính 'attribute' của đối tượng 'self' với giá trị 'value'.
    # Tóm lại : phương thức này dùng để update dữ liệu cho đối tượng khi user nhập dữ liệu mới.
    # self là đối tượng hiện tại (chính là đối tượng AnimeItem đang được gọi phương thức update)
    # attribute là tên thuộc tính của đối tượng (title, release_date, image, rating, link)
    # value là giá trị mới của thuộc tính
